...
open class PDView : View {
    ...
    protected val sizeManager get() = PDSizeStorageImpl(context).run { PercentSizeManager(screenParams) }
    ...
    private fun initSizes(attrs: AttributeSet?) {
        ...
        attrs?.also {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.PDView)

            percentWidth = sizeManager.width(
                ta.getFloat(R.styleable.PDView_pd_width, 0f),
                ta.getFloat(R.styleable.PDView_pd_widthLong, 0f)
            )

            percentHeight = sizeManager.height(
                ta.getFloat(R.styleable.PDView_pd_height, 0f),
                ta.getFloat(R.styleable.PDView_pd_heightLong, 0f)
            )

            percentMarginTop = sizeManager.height(
                ta.getFloat(R.styleable.PDView_pd_marginTop, 0f),
                ta.getFloat(R.styleable.PDView_pd_marginTopLong, 0f)
            )

            percentMarginBottom = sizeManager.height(
                ta.getFloat(R.styleable.PDView_pd_marginBottom, 0f),
                ta.getFloat(R.styleable.PDView_pd_marginBottomLong, 0f)
            )
