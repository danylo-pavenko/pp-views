...
class PDSquareLottieView : LottieAnimationView {
    ...
    private fun initSizes(attrs: AttributeSet?) {
        attrs?.also {
            ...
            val ta = context.obtainStyledAttributes(attrs, R.styleable.PDSquareLottieView)

            percentMarginStart =
                sizeManager.width(ta.getFloat(R.styleable.PDSquareLottieView_pd_marginStart, 0f))
            percentMarginEnd =
                sizeManager.width(ta.getFloat(R.styleable.PDSquareLottieView_pd_marginEnd, 0f))

            percentHeight = sizeManager.height(
                ta.getFloat(R.styleable.PDSquareLottieView_pd_height, 0f),
                ta.getFloat(R.styleable.PDSquareLottieView_pd_heightLong, 0f)
            )

            percentMarginTop = sizeManager.height(
                ta.getFloat(R.styleable.PDSquareLottieView_pd_marginTop, 0f),
                ta.getFloat(R.styleable.PDSquareLottieView_pd_marginTopLong, 0f)
            )

            percentMarginBottom = sizeManager.height(
                ta.getFloat(R.styleable.PDSquareLottieView_pd_marginBottom, 0f),
